...
async function initiateStorageAndUI() {
  ...
  const debugLog = await getOrInitProperty("debugLog", false);
...
function updateValues() {
  ...
  const debugLog = document.getElementById("debugLog_checkbox").checked;
  ...
  chrome.storage.sync.set(
    ...
    function () {
      console.log(
        `updated values: deckName: ${deckName}, modelName: ${modelName}, tagString: ${tagString}, debugLog: ${debugLog}`
      );
    }
  );
}
...
function refreshPopup(holder) {
  ...
  document.getElementById("debugLog_checkbox").checked = !!holder.debugLog;
}
